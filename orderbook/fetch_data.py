...
def fetch_nse_orders_range(start_date, end_date):
  ...
  try:
    ...
    df["Date"] = pd.to_datetime(df["sort_date"]).dt.strftime("%Y-%m-%d")
    return df
...
def run_data_pipeline():
  ...
  for _, r in orders.iterrows():
    ...
    # Save raw filtered announcements
  ...
  with open("nse_orders_data.json", "w", encoding="utf-8") as f:
    json.dump(final_output, f, indent=4)

  print("Data pipeline executed successfully. Saved to 'nse_orders_data.json'.")
