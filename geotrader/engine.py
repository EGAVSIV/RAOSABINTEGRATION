...
def main():
    ...
    for sec in all_sectors:
        ...
        if not rows:
            continue
        rows_desc = sorted(rows, key=lambda r: r["change"], reverse=True)
        if len(rows) <= 6:
            # Small sector: show every stock once instead of a top5/bottom5
            # split that would just repeat the same handful of symbols twice.
            sec_top5, sec_bottom5 = rows_desc, []
        else:
            sec_top5 = rows_desc[:5]
            sec_bottom5 = sorted(rows, key=lambda r: r["change"])[:5]
        sector_stocks[sec] = {
            "top5": sec_top5,
            "bottom5": sec_bottom5,
            "count": len(rows),
